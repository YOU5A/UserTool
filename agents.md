...
| `deleteAllUsers()` | 删除所有用户（含 outbox 写入以同步删除到云端） |
...
- `fetchAllUserData(client, ownerId)` — 分页拉取全部用户数据（1000 条/页）
- `upsertUserData(client, ownerId, userId, data, localTs)` — 条件写入单条数据
...
- **Pull:** 登录初始化 / window focus / Realtime 收到变更 / 手动刷新 → 分页拉取（1000 条/页），比较 `__ts` 取最新
- **Realtime:** Supabase channel `user_data_changes` 监听 INSERT/UPDATE/DELETE
- **条件写入:** 仅当本地 `__ts > 远程 updated_at` 时才覆盖云端数据
- **deleteAllUsers:** 为每个用户创建 delete outbox 条目，syncPushToCloud 逐条删除云端数据
...
App mount → useSupabaseAuth（自动检测配置和会话）
  ...
  │     → restoreSession → mgr.init(userId) → doPull → setupRealtime
...
signOut → syncPushToCloud → client.auth.signOut() → 清空内存数据 → App 检测未登录 → mgr.init("local_offline") → 重渲染
...
| `LogsModal` | 操作日志 |
| `SettingsModal` | 设置面板（含导出/导入/货币切换/删除全部用户/主题/Supabase 配置） |
...
- **旧用户:** 4 位尾号，`id="old-{tail}"`，添加时自动检查重复，可选备注和卡号
- **卡号:** 4 位数字（0001-9999），输入时自动提取前 4 位数字并补零，`normalizeCardNo` + `isValidCardNo` 校验
- **置顶:** `togglePin(userId)` 切换 pinned 状态，置顶用户排在前面
- **备注/卡号更新:** `updateUserInfo(userId, { remark?, cardNo? })` 单独更新
- **软删除:** `deleteUser` → `deleted=true` + `_deletedAt`，移入 trash[]；`purgeTrashItem` 设置 `purged=true` 不可恢复
- **全量删除:** `deleteAllUsers()` → 清空内存 + IndexedDB + 为每个用户创建 delete outbox 供同步推送
